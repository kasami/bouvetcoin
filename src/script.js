...
class Block {
    ...
    mineBlock(difficulty) {
        ...
        console.log("Klarte å mine blokk: " + this.hash);
        console.log("Dette tok " + this.nonce + " forsøk (" + (Math.floor((Date.now() - this.timestamp)/10)/100) + " sekunder).")
    }
...
class BlockChain {
    constructor() {
        this.chain = [];
        this.difficulty = 3;
        var genesis = this.createGenesisBlock();
    ...
    createGenesisBlock() {
        console.log("Oppretter Bouvetcoin Genesis block...");
        return new Block(0, "BouvetCoin Genesis block", "00000000000000000000000000000000");
    }

    isChainValid() {
        if (this.chain[0].previousHash !== "00000000000000000000000000000000") {
            console.log("Ugyldig genesis block hash seed");
            return false;
        }
        ...
        for(let i = 1; i < this.chain.length; i++) {
            if (this.chain[i].previousHash !== this.chain[i - 1].hash) {
                console.log("Ugyldig forrige hash i index " + i);
                console.log("Hash....: " + this.chain[i].this.chain[i].previousHash);
                console.log("Expected: " + this.chain[i - 1].hash);
                return false;
            }
...
function updateUI() {
    if (window.bouvetCoin === undefined) {
        return;
    }
...
$( document ).ready(function() {
    ...
    jQuery("#startmining").click(function () {
        ...
        console.log("Miner blokk " + bouvetCoin.getNextIndex() + " med vanskelighetsgrad " + bouvetCoin.difficulty + "...");
        bouvetCoin.addBlock(
            new Block(
        ...
        jQuery("#startmining").attr("disabled", false);
        updateUI();
    })
